...
So we are borrowing the initVariant() ideas here (by using weak gcc references). You can now define
earlyInitVariant(), postI2CInitVariant(), and lateInitVariant() if your board needs them.
earlyInitVariant() runs at the beginning of setup() directly after waitUntilPowerLevelSafe();
postI2CInitVariant() runs after the default I2C bus starts; lateInitVariant() runs after the LoRa radio is
initialized.
